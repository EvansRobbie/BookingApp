...
const PhotoUploads = ({addedPhotos, setAddedPhotos }:photosProps) => {
  const [photoLink, setPhotoLink] = useState("");
    ...
    const addPhotoByLink = async (e: React.FormEvent<HTMLButtonElement>) =>{
      ...
      const removeImage = (e: React.MouseEvent<HTMLButtonElement, MouseEvent>, link:string)=>{
        e.preventDefault()
        setAddedPhotos([...addedPhotos.filter((photo)=> photo !== link)])
      }

      const SetMainPhoto = (e: React.MouseEvent<HTMLButtonElement, MouseEvent>, link:string) =>{
        e.preventDefault() 
         ...
         <div className="flex gap-2">
            ...
            <div className="grid grid-cols-3 gap-2 md:grid-cols-4 lg:grid-cols-6 mt-2">
                {addedPhotos.length > 0 && addedPhotos.map((link) =>(
                    <div key={link} className="h-32 w-full relative">
                       {/* <img className="rounded-2xl w-full hover:scale-105 duration-500 transition-all ease-in  h-full" src={`http://localhost:4000/uploads/${link}`}/> */}
                        <Image className="rounded-2xl w-full hover:scale-105 duration-500 transition-all ease-in  h-full" src={link}/>
                        <button onClick={(e) =>removeImage(e,link)} className='absolute bottom-1 right-1 bg-black cursor-pointer text-white py-1.5 px-2 md:py-2 md:px-3 rounded-xl bg-opacity-50'>
                        ...
                        <button onClick={(e) =>SetMainPhoto(e,link)} className='absolute bottom-1 left-1 bg-black cursor-pointer text-white py-1.5 px-2 md:py-2 md:px-3 rounded-xl bg-opacity-50'>
                          {link === addedPhotos[0] ? 
