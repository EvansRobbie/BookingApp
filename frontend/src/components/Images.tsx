...
const Images = ({images, id}: {images: string[], id:string}) => {
  ...
  const ImageElement = images.map((image, index)=>{
    ...
    <div key={index} onClick={()=> goToNext(index)} className={ `${currentIndex === index ? 'text-white' : 'text-gray-200'} flex px-[0.055rem] `}>
  ...
  return (
    ...
    <div className='absolute top-2 right-4 text-white cursor-pointer active:scale-95 z-30 '>
        <svg xmlns="http://www.w3.org/2000/svg" fill="rgb(0, 0, 0, 0.5)" viewBox="0 0 24 24" strokeWidth={1.5} stroke="currentColor" className="w-8 h-8 md:w6 md:h-6">
